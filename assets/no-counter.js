//topbar counter
let domTopId = document.getElementById('no___js-counter');
if (domTopId !== null) {
...
function calculateDeliveryTimeEveryDay(someId, currentDate, setTime) {
	...
	function tick() {
		...
		if (now <= startDateOfToday) {
			...
			someId.innerHTML = `<div class="timer__wrapper">
													<div class="timer__text">
														Vi sender i morgen – Bestill innen: 
													</div>
													<div class="timer__counter">
													<span class="time-up"  >
														<span class="time-hour"> <strong>${hours} </strong><span class="time__badge">time</span></span> <code>:</code>  
...
function calculateDeliveryTimeFriday(someId, currentDate, setTime) {
	...
	function tick() {
		...
		if (now <= startDateOfToday) {
			...
			someId.innerHTML = `<div class="timer__wrapper"><div class="timer__text">Hos Anodyne® tilbyr vi alltid 100% fornøydgaranti og 30 dagers full returrett.</div></div>`;
		}
	}
	tick();
}
